...
class _ConvBlock(nn.Module):
    def __init__(
        ...
        self.spat_conv = Conv2dWithConstraint(
            F1, F1 * D, (in_channels, 1), bias=False, groups=F1, max_norm=1.0
        )
...
class TCNBlock(nn.Module):
    def __init__(
        ...
        self.conv1 = CausalConv1d(
            n_filters, n_filters, kernel_size=kernel_length, dilation=dilation
        )
...
class ATCNet(nn.Module):
    def __init__(
        ...
        self.conv_block = _ConvBlock(
            F1, kernel_length_conv, pool_length, D, in_channels, dropout_conv
        )
    ...
    def forward(self, x):
        ...
        for i, blk in enumerate(self.atc_blocks):
            blk_output = blk_output + blk(
                x[:, i : (seq_len - self.n_windows + i + 1), :]
            )
